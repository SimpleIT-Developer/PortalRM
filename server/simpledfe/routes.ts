...
import { NFSeEmissaoService } from "./services/nfse-emissao-service";
...
// Middleware to verify JWT token - MOCKED for PortalRM Integration
function authenticateToken(req: any, res: any, next: any) {
  // Mock user for integration with PortalRM to preserve functionality of existing routes
  // that depend on req.user
  req.user = { 
    id: 1, 
    type: 'admin', 
    username: 'portalrm_user', 
    email: 'admin@portalrm.com.br' 
  };
  next();
}

export async function registerSimpleDFeRoutes(app: Express) {
  // Inicializar tabelas no início - DISABLED for PortalRM Integration
  /*
  try {
  ...
  */
  ...
  // Login endpoint (DISABLED)
  app.post("/api/auth/login_disabled", async (req, res) => {
    try {
  ...
  // Get current user endpoint - returns mocked user aligned with PortalRM auth
  app.get("/api/auth/me", authenticateToken, async (req: any, res) => {
    try {
      const { id, type, username, email } = req.user;
      res.json({ user: { id, type, username, email, name: username, status: 1 } });
    } catch (error) {
  ...
  // Endpoint para verificar DPS na Sefin Nacional
  app.get("/api/dps/verificar/:dpsNumber", authenticateToken, async (req: any, res) => {
    try {
      const { dpsNumber } = req.params;
      
      console.log(`[ROUTES] Verificando DPS: ${dpsNumber}`);
      
      if (!dpsNumber || !dpsNumber.startsWith('DPS')) {
        return res.status(400).json({ 
          error: "Número de DPS inválido. Deve começar com 'DPS'" 
        });
      }
      
      // Chamar o serviço para verificar o DPS
      const resultado = await ERPService.verificarDPSExistente(dpsNumber);
      
      console.log(`[ROUTES] Resultado da verificação:`, resultado);
      
      // Log de auditoria
      await AuditLogger.logMenuAccess(req, `Verificação DPS - ${dpsNumber}`);
      ...
      console.error("[ROUTES] Erro ao verificar DPS:", error);
      res.status(500).json({ 
        error: "Erro ao verificar DPS na Sefin Nacional",
        details: error instanceof Error ? error.message : "Erro desconhecido"
      });
    }
  });

  // Endpoint para encontrar próximo DPS disponível
  app.get("/api/dps/proximo-disponivel/:dpsBase", authenticateToken, async (req: any, res) => {
    try {
      const { dpsBase } = req.params;
      const { maxTentativas = "100" } = req.query;
      
      console.log(`[ROUTES] Procurando próximo DPS disponível a partir de: ${dpsBase}`);
      
      if (!dpsBase || !dpsBase.startsWith('DPS')) {
        return res.status(400).json({ 
          error: "Número base de DPS inválido. Deve começar com 'DPS'" 
        });
      }
      
      // Chamar o serviço para encontrar DPS disponível
      const dpsDisponivel = await ERPService.encontrarDPSDisponivel(
        dpsBase, 
        parseInt(maxTentativas as string)
      );
      
      if (dpsDisponivel) {
        console.log(`[ROUTES] DPS disponível encontrado: ${dpsDisponivel}`);
        
        // Log de auditoria
        await AuditLogger.logMenuAccess(req, `DPS Disponível - ${dpsDisponivel}`);
        
        res.json({
          success: true,
          dpsDisponivel,
          message: `DPS disponível encontrado: ${dpsDisponivel}`
        });
      } else {
        console.log(`[ROUTES] Nenhum DPS disponível encontrado`);
        res.status(404).json({
          success: false,
          error: "Nenhum DPS disponível encontrado nas tentativas realizadas"
        });
      }
      
    } catch (error) {
      console.error("[ROUTES] Erro ao encontrar DPS disponível:", error);
      res.status(500).json({ 
        error: "Erro ao procurar DPS disponível",
        details: error instanceof Error ? error.message : "Erro desconhecido"
      });
    }
  });

  // Endpoint para emitir NFSe com verificação automática de DPS
  app.post("/api/nfse/emitir", authenticateToken, async (req: any, res) => {
    try {
      const {
        cnpjPrestador,
        cnpjTomador,
      ...
      // Validação básica
      if (!cnpjPrestador || !cnpjTomador || !valorServico || !descricaoServico) {
        return res.status(400).json({
          success: false,
          error: "Campos obrigatórios faltando: cnpjPrestador, cnpjTomador, valorServico, descricaoServico"
        });
      }

      console.log(`[ROUTES] Iniciando emissão de NFSe para prestador ${cnpjPrestador}`);

      // Chamar serviço de emissão
      const resultado = await NFSeEmissaoService.emitirNFSe(
        {
          cnpjPrestador,
          cnpjTomador,
          ...
          descricaoServico,
          dpsNumber,
          municipioPrestacao: municipioPrestacao || "3550308", // São Paulo por padrão
          serie: serie || "1",
          numeroNota,
          reciboId
        },
      ...
      if (resultado.success) {
        console.log(`[ROUTES] NFSe emitida com sucesso! DPS: ${resultado.dpsNumber}`);
        
        res.json({
          success: true,
          message: resultado.message,
          dpsNumber: resultado.dpsNumber,
          chaveAcesso: resultado.chaveAcesso,
          protocolo: resultado.protocolo,
          xmlGerado: resultado.xmlGerado
        });
      } else {
        console.log(`[ROUTES] Falha na emissão: ${resultado.erro}`);
      ...
      console.error("[ROUTES] Erro ao emitir NFSe:", error);
      res.status(500).json({
        success: false,
        error: "Erro ao processar emissão de NFSe",
        details: error instanceof Error ? error.message : "Erro desconhecido"
      });
    }
  });
  ...
  */
  ...
  app.get("/api/users/preferences", authenticateToken, async (req: any, res) => {
    try {
      ...
      // res.json(user[0]);
      res.json({ showVersionNotifications: true });
    } catch (error) {
