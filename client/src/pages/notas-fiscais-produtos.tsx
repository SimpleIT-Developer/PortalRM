...
interface NotaFiscalProdutoItem {
  CODCOLIGADA: number;
  IDMOV: number;
  NSEQITMMOV: number;
  IDPRD: number;
...
export default function NotasFiscaisProdutos() {
  const [data, setData] = useState<NotaFiscalProduto[]>([]);
  ...
  const [selectedNf, setSelectedNf] = useState<NotaFiscalProduto | null>(null);
  const [nfItems, setNfItems] = useState<NotaFiscalProdutoItem[]>([]);
    ...
    try {
      ...
      if (!movements || movements.length === 0) {
        toast({
          ...
          description: "Configure os movimentos de Nota Fiscal de Produto no Editar Ambiente.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!response.ok) {
        throw new Error("Falha ao carregar notas fiscais de produtos");
      }
      ...
      console.error("Erro ao buscar notas fiscais de produtos:", error);
      toast({
        title: "Erro",
        description: error.message || "Não foi possível carregar as notas fiscais de produtos.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleViewItems = async (nf: NotaFiscalProduto) => {
    setSelectedNf(nf);
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col p-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex-none flex items-center justify-between">
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground text-sm">
            Visualize as notas fiscais de produtos emitidas, filtrando por período.
          </p>
        </div>
      ...
      <Card className="border-muted/20 shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Selecione o período para buscar as notas fiscais de produtos.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center h-24 text-muted-foreground">
                    Use os filtros para buscar notas fiscais de produtos.
                  </TableCell>
                </TableRow>
              ) : (
                data.map((item, index) => (
