...
export default function OrdemCompras() {
  ...
  const [poItems, setPoItems] = useState<PurchaseOrderLineItem[]>([]);
    ...
    try {
      ...
      if (!token) {
        toast({
          ...
          description: "Faça login novamente para buscar as ordens de compra.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!movements || movements.length === 0) {
        toast({
          ...
          description: "Configure os movimentos de Ordem de Compra no Editar Ambiente.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!filtered || filtered.length === 0) {
        toast({
          ...
          description: "Não foram encontradas ordens de compra para o período informado.",
        });
      }
    } catch (error: any) {
      ...
      toast({
        title: "Erro",
        description: error.message || "Não foi possível carregar as ordens de compra.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleViewItems = async (order: PurchaseOrder) => {
    try {
      ...
      if (!token) {
        toast({
          title: "Autenticação necessária",
          description: "Faça login novamente para buscar os itens da ordem.",
          variant: "destructive",
        });
        return;
      }
      ...
      const parameters = [`CODCOLIGADA=${codColigada}`, `IDMOV=${order.IDMOV}`].join(";");
      ...
      if (!response.ok) {
        throw new Error("Falha ao carregar itens da ordem de compra");
      }
      ...
      if (!items || items.length === 0) {
        toast({
          ...
          description: "Não foram encontrados itens para esta ordem de compra.",
        });
      }
    } catch (error: any) {
      ...
      toast({
        title: "Erro",
        description: error.message || "Não foi possível carregar os itens da ordem de compra.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col p-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex-none flex items-center justify-between">
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground text-sm">
            Visualize as ordens de compra em aberto e validadas, filtrando por período.
          </p>
        </div>
      ...
      <Card className="border-muted/20 shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Selecione o período para buscar as ordens de compra.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center h-24 text-muted-foreground">
                    Use os filtros para buscar ordens de compra.
                  </TableCell>
                </TableRow>
              ) : (
                data.map((item, index) => (
      ...
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              Itens da Ordem de Compra {selectedOrder?.NUMEROMOV ?? ""}
            </DialogTitle>
            <DialogDescription>
              {selectedOrder
                ? `IDMOV: ${selectedOrder.IDMOV} | Fornecedor: ${
                    selectedOrder.CODCFO || ""
                  } - ${selectedOrder.NOME || selectedOrder.NOMEFANTASIA || ""}`
                : ""}
            </DialogDescription>
          </DialogHeader>
                ...
                {itemsLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center h-24 text-muted-foreground">
                      Nenhum item encontrado para esta ordem de compra.
                    </TableCell>
                  </TableRow>
                ) : (
