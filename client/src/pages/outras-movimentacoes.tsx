...
interface OutrasMovimentacaoItem {
  CODCOLIGADA: number;
  IDMOV: number;
  NSEQITMMOV: number;
  IDPRD: number;
...
export default function OutrasMovimentacoes() {
  const [data, setData] = useState<OutrasMovimentacao[]>([]);
  ...
  const [selectedMov, setSelectedMov] = useState<OutrasMovimentacao | null>(null);
  const [movItems, setMovItems] = useState<OutrasMovimentacaoItem[]>([]);
    ...
    try {
      ...
      if (!token) {
        toast({
          ...
          description: "Faça login novamente para buscar as movimentações.",
          variant: "destructive",
        });
        return;
      }
      ...
      const movements = EndpointService.getOtherMovements();
      if (!movements || movements.length === 0) {
        toast({
          title: "Movimentos não configurados",
          description: "Configure as Outras Movimentações no Editar Ambiente.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!response.ok) {
        throw new Error("Falha ao carregar outras movimentações");
      }
      ...
      if (!filtered || filtered.length === 0) {
        toast({
          ...
          description: "Não foram encontradas movimentações para o período informado.",
        });
      }
    } catch (error: any) {
      console.error("Erro ao buscar outras movimentações:", error);
      toast({
        title: "Erro",
        description: error.message || "Não foi possível carregar as outras movimentações.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleViewItems = async (mov: OutrasMovimentacao) => {
    setSelectedMov(mov);
    ...
    try {
      ...
      const parameters = [`CODCOLIGADA=${codColigada}`, `IDMOV=${mov.IDMOV}`].join(";");
      ...
      if (response.ok) {
        ...
        if (!items || items.length === 0) {
          toast({
            title: "Aviso",
            description: "Nenhum item encontrado para esta movimentação.",
          });
        }
      } else {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível carregar os itens da movimentação.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col p-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex-none flex items-center justify-between">
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground text-sm">
            Visualize outras movimentações emitidas, filtrando por período.
          </p>
        </div>
      ...
      <Card className="border-muted/20 shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Selecione o período para buscar as outras movimentações.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center h-24 text-muted-foreground">
                    Use os filtros para buscar outras movimentações.
                  </TableCell>
                </TableRow>
              ) : (
                data.map((item, index) => (
      ...
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              Itens da Movimentação {selectedMov?.NUMEROMOV ?? ""}
            </DialogTitle>
            <DialogDescription>
              {selectedMov
                ? `IDMOV: ${selectedMov.IDMOV} | Fornecedor: ${
                    selectedMov.CODCFO || ""
                  } - ${selectedMov.NOME || selectedMov.NOMEFANTASIA || ""}`
                : ""}
            </DialogDescription>
          </DialogHeader>
